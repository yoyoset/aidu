...
AIDU 采用典型�?Chrome Extension MV3 架构，分为后台服�?(Service Worker) 和前�?UI (Sidepanel) 两大部分，通过消息机制通信�?
...
| 模块 | 文件 | 职责 |
|------|------|------|
| **Service Worker** | `service_worker.js` | 扩展入口，监听生命周�?|
| **Pipeline Manager** | `pipeline_manager.js` | 文章分析队列管理，并发控�?|
| **LLM Client** | `llm/api_client.js` | 统一�?LLM API 调用接口 |
| **Smart Router** | `llm/smart_router.js` | Prompt 构建与策略路�?|
| **Text Chunker** | `llm/text_chunker.js` | 文本分词与清�?|
...
| 模块 | 路径 | 职责 |
|------|------|------|
| **Builder** | `features/builder/` | 文章输入与分析进度展�?|
| **Reader** | `features/reader/` | 交互式阅读与查词 |
| **Vocab** | `features/vocab/` | 生词列表与复�?|
...
| 服务 | 文件 | 职责 |
|------|------|------|
| **Draft** | `draft_service.js` | 草稿管理与存�?|

### 3. Service Layer (`src/services/`)
通用业务逻辑，可�?Background �?Sidepanel 调用�?

| 服务 | 文件 | 职责 |
|------|------|------|
| **Dictionary** | `dictionary_service.js` | 查词逻辑，缓存管�?|
| **Vocab** | `vocab_service.js` | 生词本增删改�?|
| **SRS** | `srs_algorithm.js` | 间隔重复算法 |
| **Sync** | `sync_service.js` | 多设备当前进度同�?|
