...
- **Process**:
  ...
  3. **Storage**: 保存�?`vocab_default`�?
- **Data**:
  - `{ word, lemma, meaning, phonetic, context, ... }`
  - **问题**: 保存�?`meaning` 依然是简单的短释义�?
...
## 改进方向 (Summary)
数据流的主要瓶颈在于 **阶段 2 (查词)** �?Prompt 设计限制了数据丰富度，进而影响了 **阶段 3 (保存)** 的数据质量�?

Docs 2.0 改进计划将着重于�?
1. **两段式数�?*: 区分 Tier 1 (基础) �?Tier 2 (深度)�?
2. **提前生成**: 尝试在文章分析阶段生�?Tier 1 数据�?
3. **按需增强**: 在查词阶段提�?Tier 2 深度解析入口�?
