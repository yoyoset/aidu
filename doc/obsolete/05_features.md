...
**关键交互**:
- **INPUT**: 文本框输入�?
- **PROCESS**: 提交 Draft �?PipelineManager�?
...
**功能**:
- 渲染已分析的文章，支持句子级对齐�?
- **交互式查�?*: 点击单词 (Bubble) 弹出释义�?
- **深度解析**: 在弹窗中点击 🔍 (Deep Dive) 查看词源、同义词辨析�?
- **音频播放**: 支持句子朗读�?
- **生词收藏**: �?Popover 中点�?ADD 保存生词 (自动包含搭配数据)�?
...
**功能**:
- **Profile Management (多配置管�?** (New in v4.7.0):
  - **独立隔离**: 每个 Profile 拥有独立�?API 配置 (Key/Model) 和生词本 (Vocabulary)�?
  - **跨端同步**: PC 端创建的 Profile 列表会自动同步到手机�?(Mobile PWA)�?
  - **无缝切换**: 支持在不同使用场�?(�?"工作模式", "日语学习") 间快速切换�?
  - **数据安全**: 删除 Profile 时彻底清理关联数据�?
- **AI Model Customization**:
  - 支持 DeepSeek, Google Gemini, OpenAI �?Custom Endpoint�?
  - 针对每个 Profile 独立保存模型参数�?
- **Teaching Persona**: 选择 AI 的讲解风�?(轻松、学术、幽默等)�?
- **Cloud Sync (云同�?**:
  - **双向同步**: 插件与手机端数据通过 Pull-Before-Push 策略保持一致�?
  - **自动同步**: 后台�?30 分钟静默同步活跃 Profile�?
