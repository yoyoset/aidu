...
# 3. 核心模块：UI 与交�?(Module B: UI & Interaction)

本模块实�?MVC 架构中的 View �?Controller 层，负责数据渲染与用户指令响应�?

## 3.1 侧边栏布局 (Sidepanel Layout)
- **容器**: Flexbox 布局，分�?`Header` (吸顶), `Content` (滚动), `Footer` (吸底).
- **组件生命周期**: `init()` -> `bindEvents()` -> `render()` -> `destroy()`.
- **内存管理**: `destroy()` 方法必须显式移除 Event Listeners �?Observers�?

## 3.2 准备仪表�?(Preparation Dashboard)
- **目标**: 提供 Analysis Pipeline 的可视化干预接口�?
- **交互**: 支持拖拽合并 (Merge Down) 和快捷键操作 (Enter 启动).

### 3.2.1 入口策略 (Entry Points)
1.  **扩展图标 (Extension Icon)**: 点击打开 Sidepanel，进�?Dashboard�?
2.  **全局右键菜单 (Global Context Menu)**:
    *   **Context**: Page (无选中内容�?.
    *   **Label**: "Open AIDU Dashboard".
    *   **Action**: 打开 Sidepanel，进�?Dashboard�?
3.  **选中右键菜单 (Selection Context Menu)**:
    *   **Context**: Selection (选中内容�?.
    *   **Label**: "Save to AIDU Library".
    *   **Action**: **Silent Save (静默保存)**。不打开 Sidepanel，直接后台创�?Draft (Raw Meat)，并在图标上显示 "OK" �?"Saved" 徽标反馈（如有权限）�?

### 3.2.3 Draft Creator Modal
**Entry**: Dashboard 顶部 "**+**" 按钮 �?Draft Item �?"**✏️**" 按钮�?
**Tabs**:
- **Text Tab (Default)**: Raw Text 输入�?
- **JSON Tab**: 外部 LLM 生成�?JSON 导入�?
...
**原则**: 静态结构，动态状态�?
- **HTML**: 所有主要的模态框 (Modals)、面�?(Panels) 结构必须�?`index.html` 中预定义�?
- **CSS**: 使用 CSS Modules (e.g. `dashboard.module.css`) 管理组件样式，实现样式隔离�?
- **安全**: 严禁使用 `innerHTML` 渲染用户生成内容 (UGC)，必须使�?`textContent` �?`Sanitizer` API�?

## 3.5 控制器架�?(Controller Architecture)
**模式**: Mediator / Coordinator.
- **Controller (`index.js`)**: 应用入口，负责初始化子模块依赖注�?(Dependency Injection)�?
- **MessageRouter**: 负责 Service Worker �?UI 线程�?IPC 通信�?
...
- **导入/导出 (Import/Export)**:
    ...
    - **Export**: 导出所�?draft �?`aidu_drafts_Backup.json`.
    - **Import**: 读取 JSON，根�?`draft.id` 去重 (Skip duplicates)，刷新列表�?
- **创作入口 (Draft Creator Modal)**:
    ...
    - **Tab A: New Material (新建素材)**:
        ...
        - **Action 2: Analyze Now**: 存入并立即触发分�?(Status: processing)�?
...
- **仪表盘视�?(Dashboard Views)**:
    - **View 1: Library (素材�?**: 展示 `status: draft`。等待分析�?
        ...
        - **Merge Strategy (Collection Mode)**:
            ...
            - **Interaction**: 支持 **Drag & Drop** (拖拽 A �?B) �?**Multi-Select** (复选框) 进行合并�?
            - **Result**: 合并为一个新 Draft，Title 自动拼接或重命名，Raw Text 使用 **双换�?(`\n\n`)** 连接�?
    - **View 2: Processing (处理�?**: 展示 `status: processing`。实时进度�?
    - **View 3: Completed (已完�?**: 展示 `status: ready`。点击阅读�?
    - **Navigation**: 顶部 Tabs: `Library` | `Processing` | `Completed`�?
- **草稿编辑�?(Draft Editor)**: 允许用户修正识别错误的文本�?
